...
def TemplatesToJSON(dmd, templates):
...
if __name__ == "__main__":
   ...
   for pack in dmd.ZenPackManager.packs():
       if pack.isDevelopment():
           devpacks.append(pack)
   print "Found %s Development Zenpacks." % len(devpacks)
   for i,pack in enumerate(devpacks):
       print "\t %s: %s" % (i+1,pack.id)
   input = raw_input("Which zenpack would you like to export? ")
   input = int(input)
   if (0<input<len(devpacks)+1):
  
       rrdTemplates = []
       pack = devpacks[input-1]
       ...
       data = TemplatesToJSON(dmd, rrdTemplates)
       print "JSON FOLLOWS THIS LINE."
       print data
   else:
       print "Please select an integer between 1 and %s" % len(devpacks)
       sys.exit(1)
