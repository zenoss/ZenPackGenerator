...
log = logging.getLogger("zen.TemplateLoader")
fix_ids = []
...
def YAMLFileToTemplates(dmd, yamlFile, ZenPack):
    data = None
    with open(yamlFile, 'r') as yamlFileHandle:
        data = yaml.load(yamlFileHandle)
...
class ImportTemplate(ZenScriptBase):
    ...
    def buildOptions(self):
        ...
        self.parser.add_option("-t", "--type", dest="ftype",
                help="Type of Template: [json|yaml]", metavar="TYPE")
        self.parser.set_usage("Usage: %prog -t TYPE -z ZenPack filename")
